...
u8 get_matched_pymb(u8 *strin,py_index **matchlist)
{
	...
	{
		temp=str_match(strin,(u8*)py_index3[i].py);//Zxs
		if(temp)
		{
			if(temp==0XFF){//完全匹配
...
u8 get_pymb(u8* str)
{
	return get_matched_pymb(str,g_t26.pymb);
}
...
void test_py(u8 *inputstr)
{
	u8 t=0;
	u8 i=0;
	t=g_t26.getpymb(inputstr);
	if(t&0X80)
	{
		printf("\r\n输入数字为:%s\r\n",inputstr);
		printf("部分匹配位数:%d\r\n",t&0X7F);
		printf("部分匹配结果:%s,%s\r\n",g_t26.pymb[0]->py,g_t26.pymb[0]->pymb);
	}else if(t)
	{
		printf("\r\n输入数字为:%s\r\n",inputstr);
		printf("完全匹配个数:%d\r\n",t);
		...
		for(i=0;i<t;i++)
		{
			printf("%s,%s\r\n",g_t26.pymb[i]->py,g_t26.pymb[i]->pymb);
		}
